...
@interface EditPlaceTypeViewController ()
...
@implementation EditPlaceTypeViewController
...
-(void)updateBeerWithNewId:(NSString*)newId
{
    ...
    fieldUpdateData.editedFieldName = @"placeTypeId";
    fieldUpdateData.oldValue = self.editedItem.place.placeTypeId;
    ...
    {   
         ...
         if (error)
         {
             msg = @"An error occured while updating type of place";
         }
         else
         {
             msg = @"Type of place was updated and awaits admin's approval";
             result = 666;
         }
...
{
    self.viewTitle = [NSString stringWithFormat:@"Type of '%@'", self.editedItem.place.name];
}
...
-(void)saveItem:(id)object
{
    if (self.editedItem)
    {
        if (self.switchControl.isOn && ![NSString isNullOrEmpty:object])
        {
            ...
            placeType.name = object;
            
            [[ComServices sharedComServices].placeTypeService addPlaceType:placeType onComplete:^(PlaceTypeM *placeType, NSError *error)
            {   
                 if (error)
                 {
                     [super showSaveResult:0 withMessage:@"An error occured while creating place type"];
                 }
                 else
                 {
                     [self updateBeerWithNewId:placeType.id];
                 }
             }];
        }
        else
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        cell.textLabel.text = self.editedItem.placeType.name;
    }
