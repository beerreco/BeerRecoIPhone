//
//  EditBeerCountryViewController.m
//  BeerReco
//
...
@interface EditBeerCountryViewController ()
...
@implementation EditBeerCountryViewController
...
-(void)updateBeerWithNewId:(NSString*)newId
{
    ...
    fieldUpdateData.editedFieldName = @"originCountryId";
    fieldUpdateData.oldValue = self.editedItem.beer.originCountryId;
    ...
    [[ComServices sharedComServices].beersService updateBeer:fieldUpdateData onComplete:^(NSError *error)
     {
         ...
         if (error)
         {
             msg = @"An error occured while updating origin country of beer";
         }
         else
         {
             msg = @"Origin country of beer was updated and awaits admin's approval";
             result = 666;
         }
...
{
    self.viewTitle = [NSString stringWithFormat:@"Origin of '%@'", self.editedItem.beer.name];
}
        ...
        if (self.switchControl.isOn && ![NSString isNullOrEmpty:object])
        {
            ...
            [[ComServices sharedComServices].originCountryService addOriginCountry:country onComplete:^(CountryM *country, NSError *error)
             {  
                 ...
                 {
                     [self updateBeerWithNewId:country.id];
                 }
             }];
        }
        else
...
{
    if ([segue.identifier isEqualToString:@"originCountrySelectionSegue"])
    {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        cell.textLabel.text = self.editedItem.country.name;
    }
