...
- (NSArray *)stringsFromText:(NSString *)string
{
    ...
    // checking if even the first word doesn't fit and need to elipsis on it and thats all
    BOOL oneWordTrimming = NO;
    
    while (characterArray.count != 0 && !oneWordTrimming)
    {
        NSString *line = @"";
...
//            }
            ...
            else
            {
                ...
                // Rony Lemberg: this is a manual wordWrap
                // this is done since the word wrap implemented here is wraping the last word no
                // meter if it's out of line or not which is bad!
                else if (characterArray.count - 1 == i || (![[characterArray objectAtIndex:i + 1] isEqualToString:@" "] || characterArray.count - (i + 1) > 0))
                {
                    NSString* tempLine = [NSString stringWithString:line];
                    for (int j = i; j >= 0; j--)
                    {
                        NSString *prevCharacter = [characterArray objectAtIndex:j];
                        if ([prevCharacter isEqualToString:@" "])
                        {
                            break;
                        }
                        else
                        {
                            line = [line substringToIndex:j];
                            [charsToRemove removeIndex:j];
                        }
                    }
    ...
    if (self.lineBreakMode == NSLineBreakByCharWrapping || oneWordTrimming)
    {
        slicedString = [self stringsWithCharactersWrappedFromArray:slicedString forceWrap:oneWordTrimming];
    }
    else if (self.lineBreakMode == NSLineBreakByWordWrapping)
    {
...
- (NSMutableArray *)stringsWithCharactersWrappedFromArray:(NSArray *)strings forceWrap:(BOOL)force
{
    NSMutableArray *newStrings = [NSMutableArray arrayWithArray:strings];
    
    // Just do it on the last word
    if (force)
    {
        int wordIndex = newStrings.count - 1;
        NSString *line = [newStrings objectAtIndex:(wordIndex)];
        line = [line stringByReplacingCharactersInRange:NSMakeRange(line.length - 3, 3) withString:@"..."];
        [newStrings removeObjectAtIndex:(wordIndex)];
        [newStrings insertObject:line atIndex:(wordIndex >= 0 ? wordIndex : 0)];
    }
    else if (newStrings.count > self.numberOfLines && self.numberOfLines != 0)
    {
...
- (NSMutableArray *)stringsWithCharactersWrappedFromArray:(NSArray *)strings
{
    return [self stringsWithCharactersWrappedFromArray:strings forceWrap:NO];
}
